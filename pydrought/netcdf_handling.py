...
def clip_to_ea(ds, shp_path="/mukau_jobs/gha_admin_0"):
    # read shapefile
    ...
    if ds.lon.attrs.get('axis'):
        del ds.lon.attrs['axis']
    if ds.lon.attrs.get('long_name'):
        del ds.lon.attrs['long_name']
    if ds.lon.attrs.get('standard_name'):
        del ds.lon.attrs['standard_name']
    if ds.lon.attrs.get('units'):
        del ds.lon.attrs['units']

    if ds.lat.attrs.get('axis'):
        del ds.lat.attrs['axis']
    if ds.lat.attrs.get('long_name'):
        del ds.lat.attrs['long_name']
    if ds.lat.attrs.get('standard_name'):
        del ds.lat.attrs['standard_name']
    if ds.lat.attrs.get('units'):
        del ds.lat.attrs['units']
    
    # if ds.precm.attrs.get('grid_mapping'):
    #     del ds.precm.attrs['grid_mapping']

    return ds

def create_netcdf_onevar_laea(dataarrays, variable_name, lat, lon, times, fillvalue, 
    ...
    new_dataset = create_ntdataset_from_array(dataarrays, times, lat, lon, variable_name) 
    ...
    new_dataset['3035'] = -2147483647 # dummy variable to set the crs new_dataset['3035'].attrs = crs new_dataset.lon.attrs = lon_attributes new_dataset.lat.attrs = lat_attributes new_dataset.attrs = global_attributes new_dataset[variable].attrs = var_attributes
...
def export_dataset_laea(dataset, fillvalue, variables_list, variables_units, 
                        output_file_path, output_file_name, creator, 
                        compression, config_file_path, config_file_name):
    """
    Exports a xarray dataset into a NetCDF file using a configuration file. 
    ...
    if data['variables'].keys() == variables_list:
        ...
        raise NameError('Variables in the configuration file do not correspond'/
...
def update_netcdf_from_db(file_path, file_name, dekad, variable_name, datatype, con):
    ...
    dekad_grid = create_ntnvar_dataset([smm.array], [dekad_date], 
                                             old_dataset.lat, old_dataset.lon, 
                                             [variable_name])
    ...
    dataset.close()
    export_dataset_laea(updated_dataset, -9999.0, variable_name, 
                              file_path, file_name, 
                              __author__)  
    logging.info = ("NetCDF file "+file_path+file_name+" updated")

    
def update_netcdf_ntnvar_laea(new_datasets_list, fillvalue, file_path, 
                                          output_file_name, 
                                          new_dates_list, variables_list, 
                                          variables_units):
    """
    ...
    dataset = xr.open_dataset(file_path+output_file_name, 
                              drop_variables = ['lambert_azimuthal_equal_area', 
                                                'laea', '3035'])
    ...
    new_dates_list = [np.datetime64(new_dates_list[i], 'D') for i in range(0, len(new_dates_list))]
    logging.info = ("Adding dates "+str(new_dates_list)+" to the dataset")
    updated_dataset = update_dataset(old_dataset, new_datasets_list, variables_list, new_dates_list)
    logging.info = ("Dataset updated ")
    export_dataset_laea(updated_dataset, fillvalue, variables_list, variables_units, 
                              file_path, output_file_name, 
                              __author__)  
...
def standardize_dataarray1v_CFconvention(dataset, variable_name):
    ...
    if '__xarray_dataarray_variable__' in dataset.keys():
        dataset = dataset.rename({'__xarray_dataarray_variable__':
                                      variable_name})
    return dataset
...
def create_1tdataset_from_array(array, time, lat, lon, variable_name):
    ...
    """
    dataset = xr.Dataset({variable_name: (['lat', 'lon'], array)}, 
...
def create_ntnvar_dataset(dataarrays_list, times, lat, lon, variables_list):
    ...
    """
    variables_dict = {}
    for variable_name, var_array_list in itertools.zip_longest(variables_list, 
                                                               dataarrays_list):
    ...
    dataset = xr.Dataset(variables_dict, 
                         coords={'lat': (['lat'], lat), 'lon': (['lon'], lon), 
                                 'time': (['time'], times)})
    return dataset
...
def update_dataset(dataset, new_datasets_list, variables_list, new_dates_list):
    ...
    times = dataset.time
    times_list = [np.datetime64(times.values[i], 'D') for i in range(0, 
                  len(times.values) )]
...
def export_dataset_onevar_laea(dataset, fillvalue, variable_name, 
                              file_path, output_file_name, 
                              creator):
    try:
    ...
    data = {"coordinates_attributes":{
                "lon_attributes":{"long_name":"projection_x_coordinate", 
                                  "units" : "meter", 
                                  "standard_name" : "projection_x_coordinate"}, 
                "lat_attributes":{"long_name":"projection_y_coordinate", 
                                  "units" : "meters", 
                                  "standard_name" : "projection_y_coordinate"}, 
               "time_attributes":{"long_name":"time", 
                                  "units" : "days since"+str(start_date)}}, 
               "variable_attributes":{"grid_mapping" : "lambert_azimuthal_equal_area", 
                                       "units" : "mm", }, 
            "global_attributes":{"date_created" : str(today), 
              "Source_Software" : "netcdftonetcdfCF-1.6.py", 
    ...
    dataset['3035'] = -2147483647 # dummy variable to set the crs new_dataset['3035'].attrs = crs new_dataset.lon.attrs = lon_attributes new_dataset.lat.attrs = lat_attributes new_dataset.attrs = global_attributes new_dataset[variable].attrs = var_attributes
...
def calculate_statistics(dekad_map):
    (max_value, 
     min_value, 
     mean_value, 
     std_value) = np.round_([np.nanmax(dekad_map), 
                           np.nanmin(dekad_map), 
                           np.nanmean(dekad_map), 
                           np.nanstd(dekad_map)], decimals = 2)
...
def get_dekads_from_netcdf_file(netcdf_file):
    ...
    for netcdf_day in netcdf_days:
        ...
        current_dekad = get_current_dekad_from_day(day) # is a function that returns either 1, 11 or 21
