...
class Info4img:
    ...
    def is_mtrx_coherent(self
        ...
        if mcols != icols:
            ...
            if mcols != icols:
                chk = False
                print("mtrx has", mcols, "columns instead of", icols)
        if mrows != irows:
            # remove the initally added 1
            irows = irows -1
            ...
            if mrows != irows:
                chk = False
                print("mtrx has", mrows, "rows instead of", irows)
        return chk
        ...
        if type(bbox) == list:
            if len(bbox) == 4:
                ...
                if info_out is None:
                    ...
                    return None
                info_out.bbox[0] += (info_out.range_cols[0] - self.range_cols[0]) * res_x
                info_out.bbox[2] += (info_out.range_cols[1] - self.range_cols[1]) * res_x
                info_out.bbox[3] += (info_out.range_rows[0] - self.range_rows[0]) * res_y
                info_out.bbox[1] += (info_out.range_rows[1] - self.range_rows[1]) * res_y
    ...
    def match(self  # TODO: implement it all
              , info2match
              ):
        info_out = None
        # check compatibility
        ...
        if self.bbox[1] > info2match.bbox[1]:
            print(self.bbox[1],info2match.bbox[1])
...
class TblDef4ora:
    def __init__(self
        ...
        print(self.dic_master)
        # TODO: check if tablename exists
        self.fkey = self.dic_master['ref_id_col']
        self.columnNames = c_ns  # cols to create for each band when in img2ora or mtrxs2ora
        self.titleNames = t_ns
        self.filters = []  # None
        if type(filters) == list:
            self.filters = filters
        if type(t_ns) == list:
            if len(t_ns) == len(c_ns):
                self.titleNames = t_ns
            else:
                print('invalid list of titles/comments')
...
class GrdDataManager4img(GrdDataManagerInterface):
    def __init__(self
        ...
        if os.path.exists(self.full_file_path):  # get info from existing file
            ...
            if self.info.type in ['GTiff']:
                ...
                self.info.srs = {"auth": srs.GetAuthorityName(None), "code": srs.GetAuthorityCode(None)}
                # = int(self.info.srs['code'])
                self.info.tm = list(ds.GetGeoTransform())
            gdal.Unlink(self.full_file_path)
            self.info.bbox = [self.info.tm[0]
    ...
    def mtrxs2img(self
        ...
        img = gdal_driver_mem.Create('dummy'
                                     , cols
                                     , rows
                                     , len(mtrxs)
                                     , gdal_numtype  # gdalconst.GDT_Byte
                                     )
...
class GrdDataManager4ora(GrdDataManagerInterface):
    def __init__(self
        ...
        super().__init__()
        dic_master = tabs2use[0].dic_master
        self.db_conn_string = db_conn_string
        self.tabs2use = tabs2use
        self.info = Info4img()
        self.master_tbl_name = tabs2use[0].master_tbl_name
        ...
        #self.info.bbox = dic_master['bbox']
        # TODO: check table and columns existence. If not: create table with fkey, fields in  filters. and other columns for values
        for i in range(len(tabs2use)):
            if tabs2use[i].master_tbl_name == self.master_tbl_name:
                tabs2use[i].fkey = self.fkey
            else:
                print('NO comparable grids')
                self.tabs2use = []
                self.info = None
                break
    ...
    def get_mtrxs(self
        ...
        str_frm = " FROM {} s LEFT JOIN " + self.master_tbl_name + " m ON m.id=s." + self.fkey
        '''
        ...
        '''
        str_whr = " WHERE "
        str_whr += "(m.xcol BETWEEN " + str(info_adapted.range_cols[0]) + " AND " + str(info_adapted.range_cols[1]) + ")"
        str_whr += " AND "
        str_whr += "(m.yrow BETWEEN " + str(info_adapted.range_rows[0]) + " AND " + str(info_adapted.range_rows[1]) + ")"
        ...
        for i in range(len(self.tabs2use)):
            ...
            for j in range(len(self.tabs2use[i].titleNames)):
                ...
                sql += str_whr + " AND " + self.tabs2use[i].columnNames[j] + " IS NOT NULL"
    ...
    def save(self
        ...
        t = next(iter(mtrxs))
        (rows, cols) = mtrxs[t].shape  # take the first one only, because the others MUST have same dimensions
        if not info4img_in.is_mtrx_coherent(mtrxs[t]):  # check if matrix dimensions are compatible with requested output
            return
        ...
        master_tbl_name = self.tabs2use[0].master_tbl_name
        fkey = self.tabs2use[0].fkey
        for i in range(len(self.tabs2use)):  # TODO: check behavior on multiple tables
            # print(self.tabs2use[i].__dict__)
            for j in range(len(self.tabs2use[i].columnNames)):
                refs[i + j]['tbl_name'] = self.tabs2use[i].name
                refs[i + j]['col_name'] = self.tabs2use[i].columnNames[j]
                if self.tabs2use[i].titleNames is None:
                    refs[i + j]['comment'] =  refs[i + j]['lyr_name']
                else:
                    refs[i + j]['comment'] = self.tabs2use[i].titleNames[j]
                refs[i + j]['filters'] = self.tabs2use[i].filters
        info4sql_by_tbl = {}
        for i in range(len(refs)):
            tbl_name = refs[i]['tbl_name']
            if tbl_name not in info4sql_by_tbl:
                info4sql_by_tbl[tbl_name] = {"cols": []
                                             , "tits": []
                                             , "lyrs": []
                                             , "filters": refs[i]['filters']
                                             }
            info4sql_by_tbl[tbl_name]['cols'].append(refs[i]['col_name'])
            info4sql_by_tbl[tbl_name]['tits'].append(refs[i]['comment'])
            info4sql_by_tbl[tbl_name]['lyrs'].append(refs[i]['lyr_name'])
        pg_conn = self.open_db_conn()
        cur = pg_conn.cursor()
        for tab_name in info4sql_by_tbl:
            keys = [fkey]
            vals = ["{}"]
            if len(info4sql_by_tbl[tab_name]['filters']) > 0:
                for j in range(0, len(info4sql_by_tbl[tab_name]['filters'])):
                    arr_str = info4sql_by_tbl[tab_name]['filters'][j].split('=')
                    keys.append(arr_str[0])
                    vals.append(arr_str[1])
                sql_whr_flt = " AND ".join(info4sql_by_tbl[tab_name]['filters'])
            else:
                sql_whr_flt = ""
            sql_whr_rc = " WHERE xcol={} AND yrow={}"
            sql_upd = "UPDATE " + tab_name + " SET {} = ({})"
            sql_sel_id = "SELECT ID FROM " + master_tbl_name + sql_whr_rc
            sql_whr = " WHERE " + fkey + " IN (SELECT ID FROM " + master_tbl_name + " WHERE {})"
            sql_cnt = "SELECT count(*) qnt FROM " + tab_name + " WHERE " + fkey + "={}"
            if sql_whr_flt != "":
                sql_cnt += " AND " + sql_whr_flt
            sql_ins = "INSERT INTO " + tab_name
            sql_ins += " (" + ",".join(keys) + ", UPDATED_WHEN)"
            sql_ins += " VALUES(" + ",".join(vals) + ", now())"
            # set NULL into all matrix range
            vals = []
            for i in range(len(info4sql_by_tbl[tab_name]['cols'])):
                vals.append('NULL')
            sql_whr_mst = "xcol BETWEEN " + str(info_adapted.range_cols[0]) + " AND " + str(info_adapted.range_cols[1])
            sql_whr_mst += " AND "
            sql_whr_mst += "yrow BETWEEN " + str(info_adapted.range_rows[0]) + " AND " + str(info_adapted.range_rows[1])
            sql = sql_upd.format(",".join(info4sql_by_tbl[tab_name]['cols']), "SELECT CAST(" + ",".join(vals) + " AS INTEGER) FROM DUAL")
            sql += sql_whr
            sql = sql.format(sql_whr_mst)
            if sql_whr_flt != "":
                sql += " AND " + sql_whr_flt
            print("sql>", sql)
            cur.execute(sql)
            ...
            for r in range(rows):
                ...
                for c in range(cols):
                    ...
                    qnt_nulls = 0
                    vals = []
                    for lyr_name in info4sql_by_tbl[tab_name]['lyrs']:
                        val = mtrxs[lyr_name][r][c]
                        if np.isnan(val):
                            vals.append(None)
                            qnt_nulls = qnt_nulls + 1
                        elif val == info_adapted.np_nodataval:
                            vals.append(None)
                            qnt_nulls = qnt_nulls + 1
                        elif val in info_adapted.void_values:
                            vals.append(None)
                            qnt_nulls = qnt_nulls + 1
                        else:
                            vals.append(str(mtrxs[lyr_name][r][c]))
                    # save data only if any of the listed values is not NULL
                    if qnt_nulls < len(info4sql_by_tbl[tab_name]['cols']):
                        sql = sql_sel_id.format(xcol, yrow)
                        cur.execute(sql)
                        row = cur.fetchone()

                        # little hack to 'borrow' row values from grid_1dd table into other grid tables that are not covering the whole world e.g grid_083dd, grid_01dd cover Africa
                        # inserts id, xcol, yrow and cell retrieved from grid_1dd
                        # prevent null rows
                        # if row is None:
                        #     sql = """INSERT INTO {0} (id, xcol, yrow, cell)
                        #             SELECT id, xcol, yrow, cell
                        #             FROM GRID_1DD
                        #             WHERE xcol={1} AND yrow={2}""".format(master_tbl_name,xcol, yrow)
                            
                        #     cur.execute(sql)
                            # print('Inserted xcol:{} yrow:{}'.format(xcol, yrow))
                            # pg_conn.commit()
                          
                        # else:
                        # print('xcol:{} yrow:{}'.format(xcol, yrow))

                        # little hack to bypass null rows for grids tables that are not global
                        if row is None:
                            print(sql)
                            continue
                        else:
                            cell_id = int(row[0])
                            sql = sql_cnt.format(row[0])
                            cur.execute(sql)
                            row = cur.fetchone()
                            qnt = int(row[0])
                            if qnt == 0:
                                sql = sql_ins.format(cell_id)
                                cur.execute(sql)
                            sql = sql_upd.format(",".join(info4sql_by_tbl[tab_name]['cols']), ",".join(vals))
                            sql += " WHERE " + fkey + "=" + str(cell_id)
                            if sql_whr_flt != "":
                                sql += " AND " + sql_whr_flt
                            print("inserting",qnt)

                            copy_str.append(f'{sql}')


                            # print('Inserted vals{}'.format(",".join(vals)))
                            # print('Inserted xcol:{} yrow:{}'.format(xcol, yrow))
                            # cur.execute(sql)
            ...
            for j in range(len(info4sql_by_tbl[tab_name]['cols'])):
                continue
                if info4sql_by_tbl[tab_name]['tits'] is None:
                    comment = info4sql_by_tbl[tab_name]['lyrs'][j]
                elif info4sql_by_tbl[tab_name]['cols'][j] == info4sql_by_tbl[tab_name]['tits'][j]:
                    comment = info4sql_by_tbl[tab_name]['lyrs'][j]
                else:
                    comment = info4sql_by_tbl[tab_name]['tits'][j]
                sql = "COMMENT ON COLUMN " + tab_name + "."
                sql += info4sql_by_tbl[tab_name]['cols'][j]
                sql += " IS '" + comment + "'"
                cur.execute(sql)
        cur.close()
        pg_conn.commit()
...
class VariableMapper(object):
    ...
    def _extract_numrow(self):
        ...
        numrow = pd.read_sql("select max (yrow) from " + str(grid_name),
                             self._con)
        return numrow.values[0][0] + 1

    def _extract_numcol(self):
        grid_name = self._get_grid_table_name()
        numcol = pd.read_sql("select max (xcol) from " + str(grid_name),
                             self._con)
    ...
    def _extract_lon(self):
        grid_name = self._get_grid_table_name()
        x_query = "select ID, ROUND((SDO_GEOM.SDO_CENTROID(cell)).sdo_point.x,5) cent_x from " + str(
            grid_name) + " where yrow = 0"
    ...
    def _extract_lat(self):
        grid_name = self._get_grid_table_name()
        y_query = "select ID, ROUND((SDO_GEOM.SDO_CENTROID(cell)).sdo_point.y,5) cent_y from " + str(
            grid_name) + " where xcol = 0"
